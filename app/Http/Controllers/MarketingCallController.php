...
class MarketingCallController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('GET')) {
            ...
            return view('marketing.index', ["EMPLOYEES" => $EMPLOYEES]);
            ...
            $MarketingCalls = MarketingCall::whereHas('patient', function ($query) use ($request) {
                ...
                if ($request->input('params')) {
                    ...
                    if (array_key_exists('employee', $search_params)) {
                        $results = $this->filterArrayForNullValues($search_params['employee']);
                        if (count($results) > 0)
                ...
                ->with(['callAction', 'patient', 'patient.membershipType', 'patient.sickFund', 'patient.patientClinic', 'patient.membershipSubtype', 'patient.branch', 'patient.documentType', 'patient.city', 'patient.next_call', 'patient.last_call'])
                ->with('user')
    ...
    public function scheduleCalls(Request $request)
    {
        ...
        $call_questionnaire_id = $request->call_questionnaire_id;
            ...
            if (!in_array($callDate->englishDayOfWeek, $excludingDays)) {
                ...
                for ($i = 0; $i < $calls; $i++) {
                    $callSchedule[] = [
                        ...
                        'created_at' => now(),
                        'updated_at' => now(),
                        'call_questionnaire_id' => $call_questionnaire_id,
                    ];
