...
use App\Rules\ServiceEmailExists;
use Carbon\Carbon;
...
class ServicesController extends Controller
{
    ...
    {
        // $services = Service::with('roles')->get();
        // dd($services);
        ...
        if ($request->isMethod('POST')) {
            ...
            return DataTables::eloquent($services)
                ...
                ->addColumn('action', function ($service) {
                    $editBtn = '<a href="' . route('services.edit', ['service' => $service->id]) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px btnUpdateService">
                    ...
                    $removeBtn = '<a data-service-name="' . $service->name . '" href=' . route('services.delete', ['service' => $service->id]) . ' class="btn btn-icon btn-active-light-primary w-30px h-30px btnDeleteService"
                                >
    ...
    public function create(Request $request)
    {
        ...
        $createView = view('services.addedit_modal', [
            'id' => $id,
            'cities' => City::all(),
        ])->render();
    ...
    public function update(Request $request, Service $service)
    {
        try {
            ...
            $service->active = $request->active_c == 'on' ? 1 : 0;
            $service->save();
    ...
    public function store(Request $request)
    {
        try {
            ...
            DB::commit();
            return response()->json(['status' => true, 'message' => 'Service Created']);
    ...
    public function export(Request $request)
    {
        ...
        return $c->view();*/
        return Excel::download(new ServicesExport($request->all()), 'services.xlsx');
    }
