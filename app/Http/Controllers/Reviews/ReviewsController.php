...
use App\Rules\ReviewEmailExists;
use Carbon\Carbon;
...
class ReviewsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            return DataTables::eloquent($reviews)
                ->filterColumn('reviews.name', function ($query, $keyword) use ($request) {
                ...
                ->addColumn('action', function ($review) {
                    $editBtn = '<a href="' . route('reviews.edit', ['review' => $review->id]) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px btnUpdateReview">
                    ...
                    $removeBtn = '<a data-review-name="' . $review->name . '" href=' . route('reviews.delete', ['review' => $review->id]) . ' class="btn btn-icon btn-active-light-primary w-30px h-30px btnDeleteReview"
                                >
    ...
    public function create(Request $request)
    {
        ...
        $createView = view('reviews.addedit_modal', [
            'id' => $id,
            'cities' => City::all(),
        ])->render();
    ...
    public function update(Request $request, Review $review)
    {
        try {
            ...
            $review->active = $request->active_c == 'on' ? 1 : 0;
            $review->save();
            if ($request->has('image_url_link') && $request->image_url_link != "undefined") {
                ...
                $review->image_url = $hashName;
                $review->save();
            }
            DB::commit();
            return response()->json(['status' => true, 'message' => 'Review Updated']);
    ...
    public function store(Request $request)
    {
        try {
            ...
            DB::commit();
            return response()->json(['status' => true, 'message' => 'Review Created']);
    ...
    public function export(Request $request)
    {
        ...
        return $c->view();*/
        return Excel::download(new ReviewsExport($request->all()), 'reviews.xlsx');
    }
